            ...
            <div key={item?.start_time} className='flex grid justify-center w-full '>
              <div className='bg-[#2D1300] w-[600px] h-[400px] shadow-[0_6px_10px_4px_rgba(0,0,0,0.5)] mb-14 rounded-[30px] '>
                <div className='grid place-items-center p-4 mt-16 w-full'>
                  <Image src={`https://gateway.pinata.cloud/ipfs/${item.image}`} class="rounded-full" height={150} width={142.67} />
                ...
                {me?.role == "Host" && (
                  <div className='flex -mt-[40px] pr-12 w-full'>
                    <div className='text-right text-[#817C7C] text-[14px] ml-auto cursor-pointer ' onClick={() => handleVolun(item?.index)}>Voluteer</div>
                  </div>
                )}
